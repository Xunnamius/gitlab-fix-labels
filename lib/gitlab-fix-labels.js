...
less. If your project needs more labels, change the perPageLimit constant.`;
...
api.createDummyProject = async () =>
{
    ...
    const result = await request.andKeepRetrying(deepmerge(requestDefaults,
    {
        method: 'POST',
...
api.deleteDummyProject = () =>
{
    ...
    return request.andKeepRetrying(deepmerge(requestDefaults,
    {
        method: 'DELETE',
...
api.getProjectLabels = (projectId) =>
{
    return request.andKeepRetrying(deepmerge(requestDefaults,
    {
        ...
        qs: {
            per_page: perPageLimit, // eslint-disable-line camelcase
        }
    }));
};
...
api.deleteProjectLabels = async (projectId) =>
{
    ...
    {
        await request.andKeepRetrying(deepmerge(requestDefaults,
        {
            method: 'DELETE',
            url: api.uriFrag + `projects/${projectId}/labels`,
            qs: {
...
api.getAllProjectIds = async () =>
{
    if(!api.getAllProjectIds.ids)
    {
        ...
        do
        {
            ...
            const results = await request.andKeepRetrying(deepmerge(requestDefaults,
            {
                url: api.uriFrag + `projects`,
                qs: {
...
(async () =>
{
    ...
    if(apiStartpointUri && apiAuthToken && action && (rawTargetProjectId === KEYWORD_ALL || targetProjectId > 0 ))
    {
        ...
        finally
        {
            ...
            catch(e) { console.warn('WARNING: cleanup attempt failed'); }
        }
    }
